...
class StlReader(object):
    ...
    @classmethod
    def read(cls, f_or_fname, check=False):
        ...
        # Open file
        if isinstance(f_or_fname, file):
            f = f_or_fname
        else:
            f = open(f_or_fname, 'rb')
        try:
...
class StlAsciiReader(StlReader):
    ...
    def readFace(self, faces, check=False):
        """ readFace(faces, check=False)
...
if __name__ == "__main__":
    import sys
    print StlReader.read(sys.stdin)
